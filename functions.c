		...
		if(count_method(aux->son->value)!=0){
			...
			if(n_func==1){
				...
				//se só existe uma, verificar params
					char* name= newStr(getIdParamType(aux->son->value))	; // remove primeiro char
					char* pop = getIdParamType(aux->son->value);
					name = newStr2(name); //remove ultimo char
					 int c = 0;
